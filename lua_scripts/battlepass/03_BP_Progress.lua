...
function BattlePass.Progress.NotifyExpGain(player, amount, data)
    ...
    if data.current_level >= maxLevel then
        player:SendBroadcastMessage(string.format(
            "|cff00ff00[Battle Pass]|r +%d XP (Level MAX atteint)",
            amount))
    else
        player:SendBroadcastMessage(string.format(
            "|cff00ff00[Battle Pass]|r +%d XP (%d/%d pour niveau %d)",
...
function BattlePass.Progress.NotifyLevelUp(player, oldLevel, newLevel)
    ...
    if levelsGained == 1 then
        player:SendBroadcastMessage(string.format(
            "|cffff8000[Battle Pass]|r Niveau %d atteint! Utilisez |cff00ff00.bp claim %d|r pour réclamer votre récompense.",
            newLevel, newLevel))
    else
        player:SendBroadcastMessage(string.format(
            "|cffff8000[Battle Pass]|r %d niveaux gagnés! (Niveau %d -> %d)",
            levelsGained, oldLevel, newLevel))
        player:SendBroadcastMessage(
            "|cff00ff00[Battle Pass]|r Utilisez |cff00ff00.bp|r pour voir vos récompenses disponibles.")
    end
