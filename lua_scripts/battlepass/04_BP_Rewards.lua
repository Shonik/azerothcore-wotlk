...
function BattlePass.Rewards.GrantItemReward(player, rewardData)
    ...
    if not itemTemplate then
        return false, "Item invalide: " .. itemId
    end
    ...
    if existingCount > 0 then
        ...
        if query then
            ...
            if maxCount == 1 then
                ...
                return false, string.format("Vous possédez déjà %s (item unique)!",
                    itemLink or rewardData.reward_name)
            end

            if maxCount > 0 and (existingCount + count) > maxCount then
                local itemLink = GetItemLink(itemId)
                return false, string.format("Limite atteinte pour %s (max: %d)!",
    ...
    if not player:AddItem(itemId, count) then
        return false, "Inventaire plein! Libérez de l'espace et réessayez."
    end

    local itemLink = GetItemLink(itemId)
    local message = string.format("Reçu: %s x%d", itemLink or rewardData.reward_name, count)
...
function BattlePass.Rewards.GrantGoldReward(player, rewardData)
    ...
    if copper <= 0 then
        return false, "Montant d'or invalide"
    end
...
function BattlePass.Rewards.GrantTitleReward(player, rewardData)
    local titleId = rewardData.reward_count
    ...
    if player:HasTitle(titleId) then
        return false, "Vous possédez déjà ce titre!"
    end

    player:SetKnownTitle(titleId)

    local message = string.format("Titre débloqué: %s", rewardData.reward_name)
...
function BattlePass.Rewards.GrantSpellReward(player, rewardData)
    ...
    if not spellId or spellId <= 0 then
        return false, "Sort invalide"
    end

    if player:HasSpell(spellId) then
        return false, "Vous connaissez déjà ce sort!"
    end

    player:LearnSpell(spellId)

    local message = string.format("Sort appris: %s", rewardData.reward_name)
...
function BattlePass.Rewards.ClaimReward(player, level)
    if not BattlePass.IsEnabled() then
        return false, "Le système Battle Pass est désactivé."
    end
    ...
    if not levelData then
        return false, string.format("Le niveau %d n'existe pas.", level)
    end

    if data.current_level < level then
        return false, string.format(
            "Vous devez atteindre le niveau %d (actuel: %d).",
            level, data.current_level)
    end

    if BattlePass.DB.IsLevelClaimed(guid, level) then
        return false, string.format("Récompense niveau %d déjà réclamée!", level)
    end
    ...
    if success then
        ...
        player:SendBroadcastMessage(string.format(
            "|cff00ff00[Battle Pass]|r Niveau %d: %s", level, message))

        return true, message
    else
        player:SendBroadcastMessage(string.format(
            "|cffff0000[Battle Pass]|r Erreur: %s", message))
...
function BattlePass.Rewards.ClaimAllRewards(player)
    ...
    for _, rewardData in ipairs(availableRewards) do
        ...
        if success then
            ...
            if message and message:find("Inventaire plein") then
                inventoryFullCount = inventoryFullCount + 1
                player:SendBroadcastMessage(
                    "|cffff0000[Battle Pass]|r Inventaire plein! Libérez de l'espace pour continuer.")
                break
            else
    ...
    if successCount > 0 then
        player:SendBroadcastMessage(string.format(
            "|cff00ff00[Battle Pass]|r %d récompense(s) réclamée(s)!", successCount))
    end

    if skippedCount > 0 then
        player:SendBroadcastMessage(string.format(
            "|cffff8000[Battle Pass]|r %d récompense(s) ignorée(s) (déjà possédé/appris)", skippedCount))
    end

    if successCount == 0 and skippedCount == 0 and inventoryFullCount == 0 then
        player:SendBroadcastMessage(
            "|cffff8000[Battle Pass]|r Aucune récompense disponible à réclamer.")
    end
...
function BattlePass.Rewards.PlayerOwnsReward(player, levelData)
    ...
    elseif rewardType == 3 then
        ...
        local titleId = rewardCount
        return player:HasTitle(titleId)
...
function BattlePass.Rewards.FormatRewardDescription(levelData)
    ...
    local desc = string.format("Niveau %d: |cffffd700%s|r",
