...
local function OnPlayerLogin(event, player)
    ...
    if BattlePass.DB.IsDailyLoginAvailable(guid) then
        ...
        if exp > 0 then
            ...
            player:SendBroadcastMessage(
                "|cff00ff00[Battle Pass]|r Bonus de connexion quotidienne!")
        end
    end
    ...
    if unclaimedCount > 0 then
        player:SendBroadcastMessage(string.format(
            "|cffff8000[Battle Pass]|r Vous avez %d récompense(s) non réclamée(s)! Utilisez |cff00ff00.bp|r",
            unclaimedCount))
    end
end
...
function BattlePass.Events.OnArenaEnd(player, isWinner)
    if not BattlePass.IsEnabled() then
        return
    end

    if isWinner then
        BattlePass.Debug("Arena win for " .. player:GetName())
        BattlePass.Progress.AwardFromSource(player, "WIN_ARENA", 0)
    end
end
...
function BattlePass.Events.OnDungeonComplete(player)
    if not BattlePass.IsEnabled() then
        return
    end

    BattlePass.Debug("Dungeon complete for " .. player:GetName())
    BattlePass.Progress.AwardFromSource(player, "DUNGEON_COMPLETE", 0)
end

function BattlePass.Events.OnRaidBossKill(player, bossId)
    if not BattlePass.IsEnabled() then
        return
    end

    BattlePass.Debug("Raid boss kill for " .. player:GetName() .. " (boss: " .. tostring(bossId) .. ")")

    local exp = BattlePass.Progress.CalculateExp(player, "RAID_BOSS_KILL", bossId or 0)
    if exp == 0 then
        exp = BattlePass.Progress.CalculateExp(player, "RAID_BOSS_KILL", 0)
    end

    if exp > 0 then
        BattlePass.Progress.AwardExp(player, exp, "RAID_BOSS_KILL")
    end
end

function BattlePass.Events.OnWorldBossKill(player, bossId)
    if not BattlePass.IsEnabled() then
        return
    end

    BattlePass.Debug("World boss kill for " .. player:GetName() .. " (boss: " .. tostring(bossId) .. ")")

    local exp = BattlePass.Progress.CalculateExp(player, "KILL_WORLDBOSS", bossId or 0)
    if exp == 0 then
        exp = BattlePass.Progress.CalculateExp(player, "KILL_WORLDBOSS", 0)
    end

    if exp > 0 then
        BattlePass.Progress.AwardExp(player, exp, "KILL_WORLDBOSS")
    end
end
